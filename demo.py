...
def run_rmbg(img, sigma=0.0):
    ...
    feed = utils.cv2_resize_img(img, new_w=int(64 * round(W * k)), new_h=int(64 * round(H * k)))
...
def process(input_fg, mask, prompt, num_samples, seed, steps, a_prompt, n_prompt, 
    ...
    if input_bg is None:
        ...
        bg = utils.cv2_resize_img(input_bg, new_h=image_height, new_w=image_width)
    ...
    fg = utils.cv2_resize_img(fg, new_w=image_width, new_h=image_height)
    mask = utils.cv2_resize_img(mask, new_w=image_width, new_h=image_height)
